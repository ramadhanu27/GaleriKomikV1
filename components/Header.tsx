...
export default function Header() {
  ...
  const [searchResults, setSearchResults] = useState<SoulScanSearchResult[]>([]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        setIsSearching(true);
        const response = await fetch(`/api/soulscan/search/${encodeURIComponent(searchQuery)}`);
  ...
  return (
    <>
      <header
        className={`sticky top-0 z-50 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 backdrop-blur-md border-b border-slate-700/50 shadow-lg transition-all duration-300 ${
        ...
        <nav className="container-custom">
          <div className="flex items-center justify-between h-16">
            <Link href="/" className="flex items-center space-x-3 group">
              <div className="relative w-10 h-10 rounded-lg overflow-hidden ring-2 ring-primary-500/50 group-hover:ring-primary-500 transition-all">
                <Image src="/logo-new.jpg" alt="Galeri Komik" fill className="object-contain transition-transform group-hover:scale-110" />
              </div>
            ...
            <div className="hidden md:flex items-center space-x-2">
              {[
                ...
                { href: "/bookmark", label: "Bookmark", icon: "M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z" },
                { href: "/pdf", label: "PDF", icon: "M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" },
                { href: "/pencarian", label: "Pencarian", icon: "M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" },
              ].map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`px-4 py-2 rounded-lg font-medium transition-all ${isActive(item.href) ? "bg-primary-600 text-white shadow-lg shadow-primary-900/30" : "text-slate-300 hover:bg-slate-700/50 hover:text-white"}`}>
            ...
            <div className="flex items-center space-x-2">
              ...
              {user ? (
                <div className="relative hidden md:block">
                  <button onClick={() => setShowUserMenu(!showUserMenu)} className="flex items-center gap-2 px-3 py-2 hover:bg-slate-700/50 rounded-lg transition-all text-slate-300 hover:text-white">
                    <div className="w-8 h-8 bg-gradient-to-br from-primary-500 to-primary-600 rounded-full flex items-center justify-center overflow-hidden">
                      {user.avatar_url ? (
                        ...
                        <span className="text-white font-bold text-sm">{user.username?.charAt(0).toUpperCase() || user.email?.charAt(0).toUpperCase()}</span>
                      )}
                    </div>
                  ...
                  {showUserMenu && (
                    <div className="absolute right-0 mt-2 w-48 bg-slate-800 rounded-xl shadow-2xl border border-slate-700 py-2 z-50">
                      <Link href="/profile" className="flex items-center gap-2 px-4 py-2 text-slate-300 hover:bg-slate-700/50 hover:text-white transition-colors" onClick={() => setShowUserMenu(false)}>
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                        </svg>
                        Profil
                      </Link>
                      <Link href="/bookmark" className="flex items-center gap-2 px-4 py-2 text-slate-300 hover:bg-slate-700/50 hover:text-white transition-colors" onClick={() => setShowUserMenu(false)}>
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z" />
                        </svg>
                        Bookmark
                      </Link>
                      <Link href="/history" className="flex items-center gap-2 px-4 py-2 text-slate-300 hover:bg-slate-700/50 hover:text-white transition-colors" onClick={() => setShowUserMenu(false)}>
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <button onClick={() => setShowAuthModal(true)} className="hidden md:flex items-center gap-2 px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white font-medium rounded-lg transition-all shadow-lg">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <button onClick={toggleTheme} className="p-2.5 hover:bg-slate-700/50 rounded-lg transition-all text-slate-300 hover:text-white" aria-label="Toggle theme">
                {theme === "dark" ? (
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path
                      strokeLinecap="round"
                    ...
                    {isSearching ? (
                      ...
                      <>
                        {searchResults.map((item, index) => (
                          <button key={index} onClick={() => handleResultClick(item.url)} className="w-full flex items-center gap-3 p-3 hover:bg-slate-700/50 transition-colors text-left border-b border-slate-700/50 last:border-0">
                            <img
                              src={`/api/image-proxy?url=${encodeURIComponent(item.imageUrl)}`}
                              alt={item.title}
                              className="w-12 h-16 object-cover rounded-lg shadow-md bg-slate-700"
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-2 text-xs text-slate-400">
                                <span>{item.type}</span>
                                {item.rating && parseFloat(item.rating) <= 10 && <span>⭐ {parseFloat(item.rating).toFixed(1)}</span>}
                              </div>
                            </div>
                            {item.isHot && <span className="px-2 py-0.5 bg-red-500 text-white text-[10px] font-bold rounded">HOT</span>}
                          </button>
                        ))}
                        <div className="border-t border-slate-700 grid grid-cols-2">
                          ...
                          <Link
                            href="/pencarian"
                            className="p-3 text-center text-sm font-medium text-primary-400 hover:bg-slate-700/50 border-l border-slate-700 transition-colors"
                            onClick={() => {
                              setSearchQuery("");
                              setSearchResults([]);
                            }}>
            ...
            <div className="lg:hidden py-4 border-t border-slate-700/50">
              <div className="relative">
                <input
                  ...
                  type="text"
                  placeholder="Cari manhwa..."
                  className="w-full px-4 py-2.5 pl-10 bg-slate-700/50 border border-slate-600 rounded-lg text-white placeholder-slate-400 focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all outline-none"
                />
                <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                </svg>
          ...
          {isMenuOpen && (
            <div className="md:hidden py-4 border-t border-slate-700/50 space-y-1">
              ...
              {[
                { href: "/", label: "Beranda", icon: "M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" },
                { href: "/update-terbaru", label: "Update Terbaru", icon: "M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" },
                ...
                { href: "/pdf", label: "PDF Converter", icon: "M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" },
                { href: "/pencarian", label: "Pencarian", icon: "M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" },
              ].map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-all ${isActive(item.href) ? "bg-primary-600 text-white" : "text-slate-300 hover:bg-slate-700/50 hover:text-white"}`}
                  onClick={closeMenu}>
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={item.icon} />
                  </svg>
